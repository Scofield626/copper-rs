//! The profile extracted from a recorded run reports every operation, the
//! contract's chains and the delivered source rates.
#![cfg(feature = "std")]

use cu29::prelude::*;
use cu29_export::pgo::compute_profile;
use cu29_runtime::planner::CuContract;
...
impl CuSrcTask for Source {
    ...
    fn process(&mut self, ctx: &CuContext, output: &mut Self::Output<'_>) -> CuResult<()> {
        // Every other CopperList carries no payload: the source is "skipped".
...
#[copper_runtime(config = "tests/pgo_profile_config.ron")]
struct App {}
...
fn profile_reports_operations_chains_and_source_rates() {
    ...
    let config = cu29::config::read_configuration("tests/pgo_profile_config.ron").unwrap();
